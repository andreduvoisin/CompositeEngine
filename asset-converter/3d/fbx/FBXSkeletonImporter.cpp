#include "FBXSkeletonImporter.h"

#include "FBXValidator.h"
#include "graphics/skeleton/Skeleton.h"

#include <fbxsdk.h>
#include <iostream>

namespace CE
{
    FBXSkeletonImporter::FBXSkeletonImporter(
            fbxsdk::FbxManager* fbxManager,
            const char* fileName,
            Skeleton* outSkeleton)
        : fbxManager(fbxManager)
        , fileName(fileName)
        , outSkeleton(outSkeleton)
    {
    }

    bool FBXSkeletonImporter::LoadSkeleton()
    {
        FBXValidator validator(fbxManager, fileName);
        if (!validator.Validate())
        {
            return false;
        }

        fbxsdk::FbxImporter* importer = fbxsdk::FbxImporter::Create(fbxManager, "");
        fbxsdk::FbxScene* fbxScene = fbxsdk::FbxScene::Create(fbxManager, "");

        if (!importer->Initialize(fileName, -1, fbxManager->GetIOSettings()))
        {
            return false;
        }

        if (!importer->Import(fbxScene))
        {
            return false;
        }

        importer->Destroy();

        fbxsdk::FbxNode* rootNode = fbxScene->GetRootNode();
        if (rootNode == nullptr)
        {
            return false;
        }

        ProcessSkeletonHierarchy(rootNode, fbxScene);

        return true;
    }

    void FBXSkeletonImporter::ProcessSkeletonHierarchy(fbxsdk::FbxNode* inRootNode, fbxsdk::FbxScene* pFbxScene)
    {
        for (int childIndex = 0; childIndex < inRootNode->GetChildCount(); ++childIndex)
        {
            fbxsdk::FbxNode* currNode = inRootNode->GetChild(childIndex);
            ProcessSkeletonHierarchyRecursively(currNode, 0, -1);
        }

        std::vector<bool> isJointUsed;
        isJointUsed.resize(outSkeleton->joints.size(), false);

        const int meshCount = pFbxScene->GetSrcObjectCount<fbxsdk::FbxMesh>();
        for (int meshIndex = 0; meshIndex < meshCount; meshIndex++)
        {
            auto* currMesh = pFbxScene->GetSrcObject<fbxsdk::FbxMesh>(meshIndex);

            int numOfDeformers = currMesh->GetDeformerCount();

            for (int deformerIndex = 0; deformerIndex < numOfDeformers; ++deformerIndex)
            {
                // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
                auto* currSkin = reinterpret_cast<fbxsdk::FbxSkin*>(
                        currMesh->GetDeformer(deformerIndex, fbxsdk::FbxDeformer::eSkin));
                if (currSkin == nullptr)
                {
                    continue;
                }

                int numOfClusters = currSkin->GetClusterCount();
                for (int clusterIndex = 0; clusterIndex < numOfClusters; ++clusterIndex)
                {
                    fbxsdk::FbxCluster* currCluster = currSkin->GetCluster(clusterIndex);
                    std::string currJointName = currCluster->GetLink()->GetName();

                    size_t currJointIndex = -1;
                    for (size_t i = 0; i < outSkeleton->joints.size(); ++i)
                    {
                        if (outSkeleton->joints[i].name == currJointName)
                        {
                            currJointIndex = i;
                            break;
                        }
                    }

                    // TODO: Swap to ozz-animation way of doing this?
                    if (currJointIndex == -1)
                    {
                        std::cout << "Couldn't find joint index.\n";
                        continue; // should this break? or return even?
                    }

                    // TransformLink refers to global initial transform of the link node.
                    // TransformLink is the global transform of the bone(link) at the binding moment.
                    // In the future, if local transform is needed (instead of global), you must multiply up the chain
                    // with inverses.
                    fbxsdk::FbxAMatrix transformLinkMatrix;
                    transformLinkMatrix = currCluster->GetTransformLinkMatrix(transformLinkMatrix);

                    const fbxsdk::FbxAMatrix inverseBindPose = transformLinkMatrix.Inverse();

                    const int totalMatrixComponents = 16;
                    for (int i = 0; i < totalMatrixComponents; ++i)
                    {
                        outSkeleton->joints[currJointIndex].inverseBindPose[i / 4][i % 4] =
                                (float) inverseBindPose.Get(i / 4, i % 4);
                    }

                    unsigned int numOfIndices = currCluster->GetControlPointIndicesCount();
                    if (numOfIndices > 0)
                    {
                        isJointUsed[currJointIndex] = true;
                    }
                }
            }
        }

        size_t numJointsRemoved = 0;
        for (size_t i = 0; i < isJointUsed.size(); ++i)
        {
            if (!isJointUsed[i] && !JointHasChild(i - numJointsRemoved))
            {
                RemoveJoint(i - numJointsRemoved);
                ++numJointsRemoved;
            }
        }
    }

    bool FBXSkeletonImporter::JointHasChild(size_t index)
    {
        for (const auto& joint : outSkeleton->joints)
        {
            if (joint.parentIndex == index)
            {
                return true;
            }
        }

        return false;
    }

    void FBXSkeletonImporter::RemoveJoint(size_t index)
    {
        outSkeleton->joints.erase(outSkeleton->joints.begin() + index);

        for (size_t i = index; i < outSkeleton->joints.size(); ++i)
        {
            if (outSkeleton->joints[i].parentIndex > static_cast<short>(index))
            {
                --outSkeleton->joints[i].parentIndex;
            }
        }
    }

    void FBXSkeletonImporter::ProcessSkeletonHierarchyRecursively(
            fbxsdk::FbxNode* inNode,
            size_t myIndex,
            size_t inParentIndex)
    {
        if (inNode->GetNodeAttribute() != nullptr
            && inNode->GetNodeAttribute()->GetAttributeType() == fbxsdk::FbxNodeAttribute::eSkeleton)
        {
            Joint currJoint;
            currJoint.parentIndex = (short) inParentIndex;
            currJoint.name = inNode->GetName();
            currJoint.inverseBindPose = glm::mat4(1.F);

            outSkeleton->joints.push_back(currJoint);
        }
        for (int i = 0; i < inNode->GetChildCount(); i++)
        {
            ProcessSkeletonHierarchyRecursively(inNode->GetChild(i), outSkeleton->joints.size(), myIndex);
        }
    }
}
